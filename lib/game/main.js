...
.defines(function(){
    ...
    GameTitle = ig.Game.extend({
        ...
        init: function() {
            ...
            // spawn flying player
            this.playerFlying = ig.game.spawnEntity(EntityFlyingPlayer, (ig.system.width / 2), (ig.system.height / 2));
            
            // add clouds in background
            for (var i = -1; i < 10; i++)
            {
                var xPos = (Math.random() * 50) + (75 * i);
                var yPos = 0 - (Math.random() * 20 + 20);
                this.clouds1.push(ig.game.spawnEntity(EntityCloud, xPos, yPos, {cloud: 1}));
                this.clouds2.push(ig.game.spawnEntity(EntityCloud, xPos, yPos, {cloud: 2}));
            }
            
            // add clouds in foreground
            for (var i = -1; i < 5; i++)
            {
                var xPos = (Math.random() * 50) + (100 * i);
                var yPos = (ig.system.height - (Math.random() * 40 + 40));
                this.clouds3.push(ig.game.spawnEntity(EntityCloud, xPos, yPos, {cloud: 3}));
            }
        ...
        draw: function() {
            ...
            if (this.clouds1.length)
            {
                var cloud = null;
                for (var i = 0; i < this.clouds1.length; i++)
                {
                    this.clouds1[i].draw();
                }
            }
            ...
            // draw clouds in background
            if (this.clouds2.length)
            {
                var cloud = null;
                for (var i = 0; i < this.clouds2.length; i++)
                {
                    this.clouds2[i].draw();
                }
            }
            ...
            // draw clouds in foreground
            if (this.clouds3.length)
            {
                var cloud = null;
                for (var i = 0; i < this.clouds3.length; i++)
                {
                    this.clouds3[i].draw();
                }
            }
    ...
    GameStage = ig.Game.extend({
        ...
        draw: function() {
            ...
            // draw controls text
            var text = 'Arrow Keys to Move - X to Jump and Jetpack Boost';
